...
- 新增 Platform Capability Layer（统一能力抽象包）
  ...
  - 定义跨端接口（示例）
    - GeoCapability.getLocation(), PushCapability.subscribe(),
      PaymentCapability.pay()
  - 强制规则
    - 业务模块不依赖任何平台插件
    - CLR 不直接引用 Android/iOS/HarmonyOS SDK
    - 所有平台能力通过 Capability 接口访问
    - Capability Interface 禁止暴露 Platform
      类型（Context/Ability/Activity）
...
- 工具与组件层（kit_*/ui_*）
  ...
  - 目的：保持 UI/工具层跨端一致性
- 服务连接层（clr_*）
  - 原则：仅通过 Capability 访问平台能力，不直接引用 Android/iOS/Harmony
    SDK
  - 目的：统一平台差异收敛点，避免平台污染
- 业务模块层（app_*）
...
- HarmonyOS
  开发者官网：[https://developer.harmonyos.com](https://developer.harmonyos.com/)
- OpenHarmony
  文档中心：[https://docs.openharmony.cn](https://docs.openharmony.cn/)
- DevEco
  Studio（IDE）：[https://developer.huawei.com/consumer/cn/deveco-studio](https://developer.huawei.com/consumer/cn/deveco-studio)
- 华为应用市场审核规范：[https://developer.huawei.com/consumer/cn/doc/app](https://developer.huawei.com/consumer/cn/doc/app)
